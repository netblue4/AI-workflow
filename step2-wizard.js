...
OUTPUT FORMAT:
Use clear section headers exactly as shown below. Label every answer "Answer:" and every explanation "Reasoning:" — do not use any other label format. At the very end, output the JSON summary block exactly as specified.
...
Answer (YES = PROHIBITED — assessment ends / NO = proceed to G2):
Reasoning:
...
G2 Gate outcome (IN SCOPE / OUT OF SCOPE):

---

GATE G3 — HIGH-RISK CLASSIFICATION — ANNEX III
G3: Does this system fall within ANY of the following Annex III high-risk domains?
...
Answer (YES = HIGH RISK / NO = proceed to G4):
...
G5_Q3 — Substantial modification check (Art.25):
...
S2_1: Which categories of data subjects are involved? Select all that apply.
...
Reasoning (describe who each selected category refers to in this use case):

S2_2: Which personal data types are likely to be processed? Select all that apply.
...
Answer (list all that apply):
Reasoning (detail the specific data for each type selected):
...
If no special category data — describe policy controls required to prevent special category data from entering the system:
Answer:
...
Answer:
Reasoning (justify why this basis applies to this specific use case):
...
Answer:
Reasoning (outline the three-part LIA test: purpose, necessity, balancing):
...
S5_2: AI system explainability level:
...
Answer (list all that apply):
Reasoning (explain the specific scenario for each risk selected):
...
S11_2: Is Art.36 supervisory authority prior consultation required?
Select: No — residual risk is acceptable | Yes — Art.36 prior consultation initiated | Under assessment
Answer:
Reasoning (note: Art.36 consultation is required where residual risk remains High or Very High after mitigation):

===================================================================
CLASSIFICATION SUMMARY (JSON) — output this block exactly at the end
===================================================================
